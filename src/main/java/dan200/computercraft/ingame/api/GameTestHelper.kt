...
import dan200.computercraft.ingame.mod.TestHooks
...
class GameTestHelper(holder: TestTrackerHolder) {
    ...
    fun fail(e: Throwable) {
        tracker.fail(e)
        TestHooks.failedTests.add(tracker.testName)
    }
...
fun GameTestSequence.thenWaitUntil(task: () -> Unit, delay: Long) = addResult(task, delay)
...
fun GameTestSequence.waitUntil(predicate: () -> Boolean) = addResult({
    if (!predicate())
        throw GameTestAssertException("Waiting")
})
...
private fun TestTracker.markAsComplete() {
    try {
        ...
        TToolkitMod.LOGGER.error("Failed to mark as complete", e)
    }
}
